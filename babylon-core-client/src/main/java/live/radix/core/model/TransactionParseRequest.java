...
public class TransactionParseRequest {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `parse_mode` to the URL query string
    if (getParseMode() != null) {
      try {
        joiner.add(String.format("%sparse_mode%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getParseMode()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    // add `validation_mode` to the URL query string
    if (getValidationMode() != null) {
      try {
        joiner.add(String.format("%svalidation_mode%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getValidationMode()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    // add `response_mode` to the URL query string
    if (getResponseMode() != null) {
      try {
        joiner.add(String.format("%sresponse_mode%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getResponseMode()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
