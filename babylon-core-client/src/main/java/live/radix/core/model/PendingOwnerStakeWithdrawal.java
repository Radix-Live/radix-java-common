...
public class PendingOwnerStakeWithdrawal {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `epoch_unlocked` to the URL query string
    if (getEpochUnlocked() != null) {
      try {
        joiner.add(String.format("%sepoch_unlocked%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getEpochUnlocked()), "UTF-8").replaceAll("\\+", "%20")));
    ...
    // add `stake_unit_amount` to the URL query string
    if (getStakeUnitAmount() != null) {
      try {
        joiner.add(String.format("%sstake_unit_amount%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getStakeUnitAmount()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
