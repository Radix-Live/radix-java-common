...
public class SignificantProtocolUpdateReadinessEntry {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `readiness_signal_name` to the URL query string
    if (getReadinessSignalName() != null) {
      try {
        joiner.add(String.format("%sreadiness_signal_name%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getReadinessSignalName()), "UTF-8").replaceAll("\\+", "%20")));
    ...
    // add `signalled_stake` to the URL query string
    if (getSignalledStake() != null) {
      try {
        joiner.add(String.format("%ssignalled_stake%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getSignalledStake()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
