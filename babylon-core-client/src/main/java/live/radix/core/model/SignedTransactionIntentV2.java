...
import live.radix.core.model.IntentSignatures;
import live.radix.core.model.TransactionIntentV2;
...
public class SignedTransactionIntentV2 {
  ...
  public SignedTransactionIntentV2 addNonRootSubintentSignaturesItem(IntentSignatures nonRootSubintentSignaturesItem) {
    if (this.nonRootSubintentSignatures == null) {
      this.nonRootSubintentSignatures = new ArrayList<>();
    }
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `transaction_intent` to the URL query string
    if (getTransactionIntent() != null) {
      joiner.add(getTransactionIntent().toUrlQueryString(prefix + "transaction_intent" + suffix));
    }

    // add `transaction_intent_signatures` to the URL query string
    if (getTransactionIntentSignatures() != null) {
      joiner.add(getTransactionIntentSignatures().toUrlQueryString(prefix + "transaction_intent_signatures" + suffix));
    }

    // add `non_root_subintent_signatures` to the URL query string
    if (getNonRootSubintentSignatures() != null) {
      for (int i = 0; i < getNonRootSubintentSignatures().size(); i++) {
        if (getNonRootSubintentSignatures().get(i) != null) {
          joiner.add(getNonRootSubintentSignatures().get(i).toUrlQueryString(String.format("%snon_root_subintent_signatures%s%s", prefix, suffix,
