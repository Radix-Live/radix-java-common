...
public class CommittedIntentMetadata {
  ...
  **/
  ...
  public Boolean isIsSameTransaction() {
    return isSameTransaction;
  }
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `is_same_transaction` to the URL query string
    if (isIsSameTransaction() != null) {
      try {
        joiner.add(String.format("%sis_same_transaction%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(isIsSameTransaction()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
