...
public class TimestampedValidatorSignature {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `validator_key` to the URL query string
    if (getValidatorKey() != null) {
      joiner.add(getValidatorKey().toUrlQueryString(prefix + "validator_key" + suffix));
    }
    ...
    // add `timestamp_ms` to the URL query string
    if (getTimestampMs() != null) {
      try {
        joiner.add(String.format("%stimestamp_ms%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getTimestampMs()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
