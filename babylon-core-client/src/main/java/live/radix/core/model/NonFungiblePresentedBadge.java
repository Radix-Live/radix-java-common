...
public class NonFungiblePresentedBadge extends PresentedBadge {
  ...
  @Override
  public NonFungiblePresentedBadge type(PresentedBadgeType type) {
    this.setType(type);
    return this;
  }

  @Override
  public NonFungiblePresentedBadge resourceAddress(String resourceAddress) {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `local_id` to the URL query string
    if (getLocalId() != null) {
      try {
        joiner.add(String.format("%slocal_id%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getLocalId()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
