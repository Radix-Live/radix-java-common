...
public class GenesisLedgerProofOrigin extends LedgerProofOrigin {
  ...
  @Override
  public GenesisLedgerProofOrigin type(LedgerProofOriginType type) {
    this.setType(type);
    return this;
  }
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `genesis_opaque_hash` to the URL query string
    if (getGenesisOpaqueHash() != null) {
      try {
        joiner.add(String.format("%sgenesis_opaque_hash%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getGenesisOpaqueHash()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
