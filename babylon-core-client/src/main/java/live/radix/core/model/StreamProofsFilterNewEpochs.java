...
public class StreamProofsFilterNewEpochs extends StreamProofsFilter {
  ...
  @Override
  public StreamProofsFilterNewEpochs type(StreamProofsFilterType type) {
    this.setType(type);
    return this;
  }
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `from_epoch` to the URL query string
    if (getFromEpoch() != null) {
      try {
        joiner.add(String.format("%sfrom_epoch%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getFromEpoch()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
