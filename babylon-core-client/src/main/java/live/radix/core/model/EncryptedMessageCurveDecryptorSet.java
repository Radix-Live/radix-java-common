...
public class EncryptedMessageCurveDecryptorSet {
  ...
  **/
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `dh_ephemeral_public_key` to the URL query string
    if (getDhEphemeralPublicKey() != null) {
      joiner.add(getDhEphemeralPublicKey().toUrlQueryString(prefix + "dh_ephemeral_public_key" + suffix));
    }

    // add `decryptors` to the URL query string
    if (getDecryptors() != null) {
      for (int i = 0; i < getDecryptors().size(); i++) {
        if (getDecryptors().get(i) != null) {
          joiner.add(getDecryptors().get(i).toUrlQueryString(String.format("%sdecryptors%s%s", prefix, suffix,
