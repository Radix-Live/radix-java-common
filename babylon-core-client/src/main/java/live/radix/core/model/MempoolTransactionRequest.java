...
public class MempoolTransactionRequest {
  ...
  **/
  ...
  public MempoolTransactionRequest addPayloadHashesItem(String payloadHashesItem) {
    if (this.payloadHashes == null) {
      this.payloadHashes = new ArrayList<>();
    }
  ...
  **/
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `payload_hashes` to the URL query string
    if (getPayloadHashes() != null) {
      for (int i = 0; i < getPayloadHashes().size(); i++) {
        try {
          joiner.add(String.format("%spayload_hashes%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(String.valueOf(getPayloadHashes().get(i)), "UTF-8").replaceAll("\\+", "%20")));
        } catch (UnsupportedEncodingException e) {
          // Should never happen, UTF-8 is always supported
          throw new RuntimeException(e);
        }
