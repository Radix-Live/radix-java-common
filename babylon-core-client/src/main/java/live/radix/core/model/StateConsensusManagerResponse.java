...
import live.radix.core.model.ProtocolVersionReadiness;
...
public class StateConsensusManagerResponse {
  ...
  private List<ProtocolVersionReadiness> currentValidatorReadinessSignals = new ArrayList<>();
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `current_proposal_statistic` to the URL query string
    if (getCurrentProposalStatistic() != null) {
      joiner.add(getCurrentProposalStatistic().toUrlQueryString(prefix + "current_proposal_statistic" + suffix));
    }

    // add `current_validator_set` to the URL query string
    if (getCurrentValidatorSet() != null) {
      joiner.add(getCurrentValidatorSet().toUrlQueryString(prefix + "current_validator_set" + suffix));
    }

    // add `current_time` to the URL query string
    if (getCurrentTime() != null) {
      joiner.add(getCurrentTime().toUrlQueryString(prefix + "current_time" + suffix));
    }

    // add `current_time_rounded_to_minutes` to the URL query string
    if (getCurrentTimeRoundedToMinutes() != null) {
      joiner.add(getCurrentTimeRoundedToMinutes().toUrlQueryString(prefix + "current_time_rounded_to_minutes" + suffix));
    }

    // add `current_validator_readiness_signals` to the URL query string
    if (getCurrentValidatorReadinessSignals() != null) {
      for (int i = 0; i < getCurrentValidatorReadinessSignals().size(); i++) {
        if (getCurrentValidatorReadinessSignals().get(i) != null) {
          joiner.add(getCurrentValidatorReadinessSignals().get(i).toUrlQueryString(String.format("%scurrent_validator_readiness_signals%s%s", prefix, suffix,
