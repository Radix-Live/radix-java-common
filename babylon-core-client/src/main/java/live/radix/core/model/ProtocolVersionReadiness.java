...
import live.radix.core.model.SignallingValidator;
...
public class ProtocolVersionReadiness {
  ...
  public ProtocolVersionReadiness addSignallingValidatorsItem(SignallingValidator signallingValidatorsItem) {
    if (this.signallingValidators == null) {
      this.signallingValidators = new ArrayList<>();
    }
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `signalled_protocol_version` to the URL query string
    if (getSignalledProtocolVersion() != null) {
      try {
        joiner.add(String.format("%ssignalled_protocol_version%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getSignalledProtocolVersion()), "UTF-8").replaceAll("\\+", "%20")));
    ...
    // add `total_active_stake_proportion` to the URL query string
    if (getTotalActiveStakeProportion() != null) {
      try {
        joiner.add(String.format("%stotal_active_stake_proportion%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getTotalActiveStakeProportion()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    // add `signalling_validators` to the URL query string
    if (getSignallingValidators() != null) {
      for (int i = 0; i < getSignallingValidators().size(); i++) {
        if (getSignallingValidators().get(i) != null) {
          joiner.add(getSignallingValidators().get(i).toUrlQueryString(String.format("%ssignalling_validators%s%s", prefix, suffix,
