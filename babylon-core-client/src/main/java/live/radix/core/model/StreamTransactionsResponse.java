...
public class StreamTransactionsResponse {
  ...
  private List<LedgerProof> proofs = new ArrayList<>();
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `previous_state_identifiers` to the URL query string
    if (getPreviousStateIdentifiers() != null) {
      joiner.add(getPreviousStateIdentifiers().toUrlQueryString(prefix + "previous_state_identifiers" + suffix));
    }
    ...
    // add `proofs` to the URL query string
    if (getProofs() != null) {
      for (int i = 0; i < getProofs().size(); i++) {
        if (getProofs().get(i) != null) {
          joiner.add(getProofs().get(i).toUrlQueryString(String.format("%sproofs%s%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix))));
        }
      }
    }
