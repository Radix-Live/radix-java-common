...
import live.radix.core.model.LtsFeeFungibleResourceBalanceChange;
import live.radix.core.model.LtsFungibleResourceBalanceChange;
...
public class LtsEntityFungibleBalanceChanges {
  ...
  **/
  ...
  public LtsEntityFungibleBalanceChanges addFeeBalanceChangesItem(LtsFeeFungibleResourceBalanceChange feeBalanceChangesItem) {
    if (this.feeBalanceChanges == null) {
      this.feeBalanceChanges = new ArrayList<>();
    }
  ...
  **/
  ...
  public LtsEntityFungibleBalanceChanges addNonFeeBalanceChangesItem(LtsFungibleResourceBalanceChange nonFeeBalanceChangesItem) {
    if (this.nonFeeBalanceChanges == null) {
      this.nonFeeBalanceChanges = new ArrayList<>();
    }
  ...
  **/
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `fee_balance_change` to the URL query string
    if (getFeeBalanceChange() != null) {
      joiner.add(getFeeBalanceChange().toUrlQueryString(prefix + "fee_balance_change" + suffix));
    }

    // add `fee_balance_changes` to the URL query string
    if (getFeeBalanceChanges() != null) {
      for (int i = 0; i < getFeeBalanceChanges().size(); i++) {
        if (getFeeBalanceChanges().get(i) != null) {
          joiner.add(getFeeBalanceChanges().get(i).toUrlQueryString(String.format("%sfee_balance_changes%s%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix))));
        }
      }
    }

    // add `non_fee_balance_changes` to the URL query string
    if (getNonFeeBalanceChanges() != null) {
      for (int i = 0; i < getNonFeeBalanceChanges().size(); i++) {
        if (getNonFeeBalanceChanges().get(i) != null) {
          joiner.add(getNonFeeBalanceChanges().get(i).toUrlQueryString(String.format("%snon_fee_balance_changes%s%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix))));
        }
      }
    }
