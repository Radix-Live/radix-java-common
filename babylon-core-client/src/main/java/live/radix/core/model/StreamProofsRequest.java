...
public class StreamProofsRequest {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `max_page_size` to the URL query string
    if (getMaxPageSize() != null) {
      try {
        joiner.add(String.format("%smax_page_size%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getMaxPageSize()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
